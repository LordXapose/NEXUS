...
Built as a learning project for quantitative finance and financial engineering. **Not investment advice, and not fit for managing real money.** Data comes from free public sources with no delivery guarantees, latency guarantees, or accuracy guarantees. The risk metrics are correctly implemented but rest on assumptions — historical VaR assumes the past 252 days represent future tail risk, and the Monte Carlo assumes multivariate normal returns, which markets are famously not. Understand the model before you trust the number.
